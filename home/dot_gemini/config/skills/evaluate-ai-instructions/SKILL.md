...
description: Evaluates AI instruction/prompt files for issues that would cause an LLM to produce poor, inconsistent, or unexpected results. Use when auditing *.instructions.md files, AGENTS.md, or any global AI instruction files to ensure high LLM performance.
---
...
3. **Perform the required analyses** (report only if there is strong evidence):
   ...
   - **Cognitive Load**: Find overly complex instruction patterns (deeply nested conditions, too many competing priorities, unclear precedence). Explain why they are hard for a model to follow.
