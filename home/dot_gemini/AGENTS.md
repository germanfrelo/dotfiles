...
- Allowed types: `feat`, `fix`, `chore`, `docs`, `style`, `refactor`, `perf`, `test`, `build`, `ci`. Use `feat` for new user-facing features, `fix` for user-facing bug fixes, `style` for formatting-only changes with no semantic impact, `chore` for maintenance tasks.
...
- Commit message subject format: `type: Message title`
  ...
  - First letter after the colon and space must be uppercase.
- Avoid subjective words like "improve", "enhance", "better", or "optimise" in subjects; describe what actually changed instead.
...
- Include a body whenever the motivation is not fully captured by the type and subject line.
- The commit body must explain _why_ the change was made, not restate what changed.
...
- Never combine `git commit` and `git push` in a single compound command.
